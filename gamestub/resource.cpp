...
Resource::Resource(const char *dataPath, ResourceType ver, Language lang) {
...
void Resource::load_FIB(const char *fileName) {
	...
	File f;
	if (f.open(_entryName, "rb", _dataPath)) {
...
void Resource::load_TEXT() {
	...
	// Load game strings
	_stringsTable = 0;
	if (f.open("STRINGS.TXT", "rb", _dataPath)) {
		const int sz = f.size();
	...
	// Load menu strings
	_textsTable = 0;
	if (f.open("MENUS.TXT", "rb", _dataPath)) {
...
void Resource::load_VCE(int num, int segment, uint8_t **buf, uint32_t *bufSize) {
	*buf = 0;
	...
	if (offset != 0xFFFF) {
		...
		if (segment < count) {
			File f;
			if (f.open("VOICE.VCE", "rb", _dataPath)) {
