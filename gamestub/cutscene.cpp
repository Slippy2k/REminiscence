...
void Cutscene::op_handleKeys() {
	...
	while (1) {
		...
		switch (key_mask) {
		case 1:
			b = (_pi->dirMask & PlayerInput::DIR_UP) != 0;
			break;
		case 2:
			b = (_pi->dirMask & PlayerInput::DIR_DOWN) != 0;
			break;
		case 4:
			b = (_pi->dirMask & PlayerInput::DIR_LEFT) != 0;
			break;
		case 8:
			b = (_pi->dirMask & PlayerInput::DIR_RIGHT) != 0;
			break;
		case 0x80:
			b = _pi->space || _pi->enter || _pi->shift;
			break;
		}
		if (b) {
			break;
...
void Cutscene::initCutscene() {
	if (_id != 0xFFFF) {
		...
		uint16_t offset   = _offsetsTable[_id * 2 + 1];
		if (cutName == 0xFFFF) {
			warning("No cutscene %d", _id);
			_id = 0xFFFF;
			return;
		}
		load(cutName);
...
void Cutscene::playCutscene() {
	while (!_interrupted && !_stop && _yieldSync == 0) {
