...
int Paula::readBuffer(int16_t *buffer, const int numSamples) {
...
inline int mixBuffer(int16_t *&buf, const int8_t *data, Paula::Offset &offset, frac_t rate, int neededSamples, uint bufSize, byte volume, byte panning) {
	int samples;
	for (samples = 0; samples < neededSamples && offset.int_off < bufSize; ++samples) {
		const int32_t tmp = ((int32_t) data[offset.int_off]) * volume;
		if (stereo) {
