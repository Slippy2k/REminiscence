...
const uint8_t *_snd_ingameMusicOffset = 0;
uint16_t _sfx_curOrder, _sfx_length;
uint16_t word_304A6, word_304A8, word_304AA;

const uint16_t _sfx_periodTable[] = {
...
void music_vector(const uint8_t *p) { // 50Hz
//	if (_snd_ingameMusicToPlay == 0) goto loc_11948;
	const uint16_t *a2 = _sfx_periodTable;
	const uint8_t *a1 = _snd_ingameMusicOffset;
	if (a1 == 0) {
		a1 = p + 0x5E - 15 * 4; // 0x22
		_sfx_curOrder = 0;
		_sfx_length = 0;
	...
	if (_sfx_length != 0) {
		...
		uint16_t tmp = READ_BE_UINT16(p + 0x3C - 15 * 4); // 0x0, numOrders
...
void Sound::startIngameMusicSample() {
	...
	*(uint16_t *)0xDFF096 = d1;
	...
	d6 = a0;
	*(uint16_t *)a0 = 0;
	*(uint32_t *)(a1 + 0) = d6; // data
	*(uint16_t *)(a1 + 4) = d0 >> 1; // len
	*(uint16_t *)(a1 + 6) = d2; // period
	*(uint16_t *)(a1 + 8) = d3; // volume
	d1 |= -0x8000;
	*(uint16_t *)0xDFF096 = d1;
	d0 = 256;
	while (--d0 != 0);
